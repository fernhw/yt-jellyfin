...
download_video() {
  ...
  local info errmsg
  errmsg=$(yt-dlp --no-playlist --extractor-args "youtube:player_client=mweb,android_vr;lang=en" --dump-json "$url" 2>&1 >"$tmpjson") || true
...
")"
  ...
  yt-dlp \
    --no-playlist \
    --extractor-args "youtube:player_client=mweb,android_vr;lang=en" \
    -S "$sort_spec" \
